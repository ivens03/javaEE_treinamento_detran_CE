...
<head>
    ...
    <style>
        ...
        .form-section, .list-section { border: 1px solid #ccc; padding: 20px; border-radius: 8px;
            margin-bottom: 20px; }
        ...
        input[type="text"], select { width: 98%; padding: 8px; border: 1px solid #ddd;
            border-radius: 4px; }
        button, input[type="submit"] { padding: 10px 20px; color: white; border: none;
            cursor: pointer; border-radius: 4px; }
        .btn-salvar { background-color: #28a745;
        }
        .btn-atualizar { background-color: #007bff;
        }
        .btn-relatorio { background: lavender }
        .btn-deletar { background-color: #dc3545;
            padding: 5px 10px; }
        .btn-editar { background-color: #ffc107; color: black; padding: 5px 10px;
...
Proprietário
    <div
        ...
        <form id="management-form" action="${pageContext.request.contextPath}/management" method="POST">
            ...
            <button type="button" id="relatorio" class="btn-relatorio" onclick="gerarRelatorio()">Gerar relatório</button>
        </form>
    </div>
                ...
                <tr data-vehicle-id="${veic.id}">
                    ...
                    <td>
                        ...
                        <form action="${pageContext.request.contextPath}/management" method="POST" style="display:inline;">
                            ...
                            <input type="submit"
                                   class="btn-deletar" value="Excluir" onclick="return confirm('Tem certeza que deseja excluir o veículo ${veic.id}?');">
                        </form>
                    </td>
                </tr>
            </c:forEach>
...
<script>
    // **********************************************
    // FUNÇÃO ADICIONADA/ATUALIZADA PARA O RELATÓRIO
    // **********************************************
    function gerarRelatorio() {
        // Abre o relatório em uma nova aba, chamando o Servlet
        // com o parâmetro 'action=gerarRelatorio' para que o doGet o processe.
        window.open('${pageContext.request.contextPath}/management?action=gerarRelatorio', '_blank');
    }
    // **********************************************
    ...
    function populateForm(buttonElement) {
        ...
        fields.action.value = 'createOrUpdate';
        // A ação já é tratada no Servlet
